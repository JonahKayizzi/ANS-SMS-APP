...
/**
 * This is the model class for table "safety_logs_causes".
 *
 * The followings are the available columns in table 'safety_logs_causes':
 * @property integer $id
 * @property string $name
 * @property integer $user
 * @property string $date
 */
class SafetyLogsCauses extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('name, target, display_kpi', 'required'),
			array('user, target', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, user, date, target', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return SafetyLogsCauses the static model class
	 */
	...
	public static function getLogCount($id, $start_date, $end_date){
		if($start_date == $end_date){
			$logs=SafetyLogs::model()->findAll("cause=:causeID and date_occured = '{$start_date}'", array(':causeID'=>$id));
		}else{
			$logs=SafetyLogs::model()->findAll("cause=:causeID and date_occured between '{$start_date}' and '{$end_date}'", array(':causeID'=>$id));
		}
	...
	public static function getLogCount2($id, $start_date, $end_date, $search_option, $search_value){
		if($start_date == $end_date){
			$logs=SafetyLogs::model()->findAll("cause=:causeID and date_occured = '{$start_date}' and {$search_option} = '{$search_value}'", array(':causeID'=>$id));
		}else{
			$logs=SafetyLogs::model()->findAll("cause=:causeID and date_occured between '{$start_date}' and '{$end_date}' and {$search_option} = '{$search_value}'", array(':causeID'=>$id));
		}
	...
	public static function getLogCountForDay($id, $day){
		$logs=SafetyLogs::model()->findAll("cause=:causeID and date_occured = '{$day}'", array(':causeID'=>$id));
		return count($logs);
	}
	
	public static function getLogCountForDay2($id, $day, $search_option, $search_value){
		$logs=SafetyLogs::model()->findAll("cause=:causeID and date_occured = '{$day}' and {$search_option} = '{$search_value}'", array(':causeID'=>$id));
		return count($logs);
	}
	
	public static function getLogCountForDays($id, $date_1, $date_2){
		$logs=SafetyLogs::model()->findAll("cause=:causeID and date_occured between '{$date_1}' and '{$date_2}'", array(':causeID'=>$id));
		return count($logs);
	}
	
	public static function getLogCountForDays2($id, $date_1, $date_2, $search_option, $search_value){
		$logs=SafetyLogs::model()->findAll("cause=:causeID and date_occured between '{$date_1}' and '{$date_2}' and {$search_option} = '{$search_value}'", array(':causeID'=>$id));
		return count($logs);
	}
	...
	public static function getOptions(){
		$titles = SafetyLogsCauses::model()->findAll();
	...
	public static function getOptionsCauses(){
		$titles = SafetyLogsCauses::model()->findAll();
	...
	public function causePlotValues($id, $start_date, $end_date, $period_type){
		//$today = date('Y-m-d');
		//$today = $select_dt;
	...
	public function causePlotValues2($id, $start_date, $end_date, $period_type, $search_option, $search_value){
		//$today = date('Y-m-d');
		//$today = $select_dt;
		$y = date('Y', strtotime($today));
		$m = date('m', strtotime($today));
	...
	public static function checkIfUserIsCurrentRecordViewer(){
		...
		$current_record = SafetyLogsCauses::model()->findByPk($current_record_id);
