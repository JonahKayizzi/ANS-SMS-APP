...
class Operators extends CActiveRecord
{
	...
	public static function getOperatorOptions(){
		$operators = @Operators::model()->findAll("status=1");
		$data = array(null=>"Select operator");
		foreach($operators as $model){
	...
	public static function checkIfUserIsCurrentRecordViewer(){
		...
		$current_record = Operators::model()->findByPk($current_record_id);
