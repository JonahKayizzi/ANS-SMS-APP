...
class UserGroups extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, name, data', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'users0' => array(self::HAS_MANY, 'Users', 'user_group_id'),
		);
	}
	...
	public static function getUserGroupOptions(){
		$titles = UserGroups::model()->findAll();
		$data = array(null=>"Select User Group");
		foreach($titles as $model){
			$data[$model->id] = $model->name;
		}
		return $data;
	}
		public static function getUserGroupOptionsByNames(){
		$titles = UserGroups::model()->findAll();
		$data = array(null=>"Select User Group");
		foreach($titles as $model){
	...
	public static function checkIfUserIsCurrentRecordViewer(){
		...
		$current_record = UserGroups::model()->findByPk($current_record_id);
