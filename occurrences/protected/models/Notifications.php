...
class Notifications extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('user, target_group', 'numerical', 'integerOnly'=>true),
			array('status', 'length', 'max'=>7),
			array('date_created,type,subject', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, description, status, type,subject, date_created,type,urgent, target_group', 'safe', 'on'=>'search'),
		);
	}
	...
	public function sendGroupNotificationsConsole($users, $subject, $description, $urgent){
		$type='GROUP';
		foreach($users as $user){			
			$this->sendNotificationConsole($subject, $description, $user->id, $type, $urgent);
		}
	...
	public function sendNotificationConsole($subject, $description, $user_to, $type, $urgent, $user_from){
		//$user_from = @Yii::app()->user->id;
		$status = 'sent';
		$notification = new Notifications;
		$notification->subject = $subject;
